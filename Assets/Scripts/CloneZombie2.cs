...
public class CloneZombie2 : MonoBehaviour
{
    ...
    public int maxTotalZombies = 60;         // Total number of zombies allowed to spawn
    ...
    void SpawnZombieWave()
    {
        ...
        {
            //float range
            float randomX = UnityEngine.Random.Range(321, 330);
            float randomZ = UnityEngine.Random.Range(605, 614);
        ...
        Debug.Log("2Wave " + waveCount + " spawned " + zombiesToSpawn + " zombies.");

        //increase zombie for next wave: 50% more zombies each wave. Cap at 50max
        maxZombiesPerWave = Mathf.Min(Mathf.CeilToInt(maxZombiesPerWave * 1.25f), 60);
    }
}
